...
useEffect(()=>{
// Matikan semua overlay di belakang
const overlays=document.querySelectorAll('.modal-overlay')
overlays.forEach(el=>{el._oldPE=el.style.pointerEvents;el.style.pointerEvents='none'})
return()=>{
overlays.forEach(el=>{el.style.pointerEvents=el._oldPE||''})
}
},[])
...
<div onClick={e=>{if(e.target===e.currentTarget)onClose()}} style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.7)',zIndex:999999,display:'flex',alignItems:'center',justifyContent:'center'}}>
<div onClick={e=>e.stopPropagation()} style={{background:'white',borderRadius:16,width:'calc(100% - 32px)',maxWidth:320}}>
...
<button onClick={onClose} style={{width:32,height:32,borderRadius:8,border:'none',background:'#f3f4f6',cursor:'pointer',fontSize:16}}>✕</button>
</div>
